...
const Photo = () => {
  ...
  const handleCommentSubmit = (newComment) => {
    ...
    newCommentArray.push({"text" : newComment});
  ...
  return (
    <>
      {userPost.map((item) => (
        <div
          className="w-full bg-bggray my-2  rounded-md text-[9px] text-lightgray"
          key={item.id}
        >
          <img
          ...
          <div className=" h-min flex flex-col mx-1 p-2">
            ...
            <div className="font-semibold ml-16 border-y-[1px] border-white py-2 w-4/5">
              {item.text}
            </div>
